...
app = FastAPI(title="WiCyS Federated Client A")

CLIENT_ID = "client_a"
DATA_PATH = Path("/app/data/client_a_events.csv")
...
def extract_features(message: str):
    ...
    contains_password = int(("password" in msg) or ("passphrase" in msg) or ("reset" in msg))
    contains_urgent = int(("urgent" in msg) or ("immediately" in msg) or ("expire" in msg))
    contains_reward = int(("gift card" in msg) or ("bonus" in msg) or ("reward" in msg))
...
def train_and_submit(req: TrainAndSubmitRequest):
    try:
        ...
        payload = dict(update)
        payload["round"] = req.round  # not needed by aggregator schema, but okay locally
